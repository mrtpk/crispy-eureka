...
def get_fov_pc(pc_velo, calib, gt_img):
    """ Function that extract from the point cloud another point cloud
    containing only the points in the field of view (FOV) and the ground truth labels
     """
    height, width = gt_img.shape[:2]

    imgfov_pc_velo, pts_2d, fov_inds = get_lidar_in_image_fov(pc_velo, calib, 0, 0, width, height, return_more=True)
    ...
    gt_labels = gt_img[velo_to_pxls[:, 1], velo_to_pxls[:, 0]].sum(1) == sum([255, 0, 255])

    # TODO: PyntCloud is not in the requirements of the project. Should we add it?
    # retrieve the points in the field of view of the camera
    fov_cloud = PyntCloud(pd.DataFrame(pc_velo[fov_inds, :], columns=['x', 'y', 'z']))

    # assign labels to points according if they belong to the road or not
    fov_cloud.points['ground_truth'] = gt_labels.astype(int)
